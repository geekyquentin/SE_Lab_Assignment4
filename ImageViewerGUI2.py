...
# global variable for the selected option
selected_option = None


# Display selected and modified images
def DisplayImages(selected_image, modified_image):
    DisplaySelectedImage(selected_image)
    DisplayModifiedImage(modified_image)


# Display the selected image
def DisplaySelectedImage(selected_image):
...
def DisplayModifiedImage(modified_image):
...
def fileClick(clicked, dataset, segmentor):
    ...
    if clicked.get() == "Segmentation":
        DisplayImages(selected_image, segmented_image)
    elif clicked.get() == "Bounding-box":
        DisplayImages(selected_image, bboxed_image)
...
# Change the image when the dropdown button is changed
def changeModifyImage():
    ...
    if clicked.get() == "Segmentation":
        segmented_image = Image.open(path + 'segmented_images/' + index + '.png')

        # Display the modified image
        DisplayModifiedImage(segmented_image)
    elif clicked.get() == "Bounding-box":
        bboxed_image = Image.open(path + 'bounding_boxes/' + index + '.png')

        # Display the modified image
        DisplayModifiedImage(bboxed_image)
