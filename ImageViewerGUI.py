...
class IM:
    ...
    def DisplayImages():
        IM.DisplaySelectedImage()
        IM.DisplayModifiedImage()
    ...
    def DisplaySelectedImage():
        ...
        selected_image = ImageTk.PhotoImage(IM.selected_image)
    ...
    def DisplayModifiedImage():
        # Set the modified image
        if IM.selected_option == "Segmentation":
            modified_image = IM.segmented_image
        elif IM.selected_option == "Bounding-box":
            modified_image = IM.bboxed_image
...
def fileClick(clicked, dataset, segmentor):
    ...
    IM.selected_image = Image.open(file_browser)
    ...
    IM.segmented_image, IM.bboxed_image = plot_visualization(data_item, image.transpose((1, 2, 0)), index, pred_masks)

    # Update the selected option
    IM.selected_option = clicked.get()
...
def process(clicked):
    ...
    if(IM.selected_image is None):
        print("Select an image to process")
        return

    # check if the clicked option is changed
    if(clicked.get() != IM.selected_option):
        # Update the selected option
        IM.selected_option = clicked.get()
